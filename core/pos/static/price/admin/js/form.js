...
var price = {
    ...
    calculateInvoice: function () {
        ...
        var change = cash - price.detail.total;
...
$(function () {
        ...
        source: function (request, response) {
            $.ajax({
                ...
                data: {
                    ...
                    'term': request.term,
                    'ids': JSON.stringify(price.getProductIds()),
                },
                dataType: "json",
                type: "POST",
                headers: {
    ...
    $('.btnRemoveAllProducts').on('click', function () {
        if (price.detail.products.length === 0) return false;
        dialog_action({
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            price.detail.products[tr.row].cant = parseInt($(this).val());
            price.calculateInvoice();
            $('td:last', tblProducts.row(tr.row).node()).html('$' + price.detail.products[tr.row].total.toFixed(2));
        })
        .on('change', 'input[name="dscto_unitary"]', function () {
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            price.detail.products[tr.row].dscto = parseFloat($(this).val());
            price.calculateInvoice();
            var parent = $(this).closest('.bootstrap-touchspin');
            parent.find('.bootstrap-touchspin-postfix').children().html(price.detail.products[tr.row].total_dscto.toFixed(2));
            $('td:last', tblProducts.row(tr.row).node()).html('$' + price.detail.products[tr.row].total.toFixed(2));
        })
        .on('click', 'a[rel="remove"]', function () {
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            price.detail.products.splice(tr.row, 1);
    ...
    $('#frmForm').on('submit', function (e) {
        e.preventDefault();
        if (price.detail.products.length === 0) {
        ...
        var form = $(this)[0];
        var params = new FormData(form);
        params.append('products', JSON.stringify(price.detail.products));
